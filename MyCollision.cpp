...
bool IsCollision(const Sphere& sphere, const Plane& plane) {
	...
	return std::fabs(distanceFromCenterToPlane) <= sphere.radius;
}
...
bool IsCollision(const OBB& obb1, const OBB& obb2) {
	...
	for (int i = 0; i < 15; ++i) {
		...
		/*float r1 = 0, r2 = 0;
		for (int j = 0; j < 3; ++j) {
			r1 += Dot(obb1.size, Vector3(std::abs(obb1.orientations[j].x), std::abs(obb1.orientations[j].y), std::abs(obb1.orientations[j].z)));
			r2 += Dot(obb2.size, Vector3(std::abs(obb2.orientations[j].x), std::abs(obb2.orientations[j].y), std::abs(obb2.orientations[j].z)));
		}*/
